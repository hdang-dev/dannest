 ...
 *
 * <p>The backend does no image processing — it only validates the file, uploads
 * the bytes, records the asset, and returns its id + public URL. Cropping/zooming
 * stays client-side; we only persist the resulting crop rectangle.
 */
@Service
@Transactional
...
public class MediaService {
     ...
     * keep resolving from their own snapshot) and, for uploads, actually free the bytes in R2.
     */
